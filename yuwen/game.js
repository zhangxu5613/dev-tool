/* ============================================================
 * 语文练习主逻辑：流程控制 / 答题 / 错题本 / 结算
 * 依赖 generator.js 暴露的 window.YuwenGen
 * ============================================================ */
(function () {
  "use strict";

  const $ = (id) => document.getElementById(id);
  const WRONG_KEY = "yuwen_wrong_book_v1";
  const SCORE_KEY = "yuwen_total_correct_v1";
  ...
  function renderQuestion() {
    ...
    q.choices.forEach((c) => {
      ...
      // 多字选项（如词语）降字号，避免拥挤
      if (String(c).length > 1) btn.classList.add("multi");
      btn.textContent = c;
  ...
  function finishRound() {
    ...
    if (state.roundWrong.length) {
      ...
      state.roundWrong.forEach((q) => {
        ...
        li.innerHTML = `<b>[${escapeHTML(q.typeName)}]</b> ${escapeHTML(q.prompt)}　→　答案 <b>${escapeHTML(q.answer)}</b>`;
        ul.appendChild(li);
      });
    } else {
  ...
  function bind() {
    $("btn-start").addEventListener("click", () => {
      ...
      const qs = YuwenGen.generateQuestions(types, getLevel(), getCount());
    ...
    $("btn-review").addEventListener("click", () => {
      const list = loadWrong();
      if (list.length === 0) { alert("错题本是空的，先去练习吧！"); return; }
      const qs = YuwenGen._utils.shuffle(list).map((q) => ({
        ...q, choices: YuwenGen._utils.shuffle(q.choices)
      }));
    ...
    $("btn-again").addEventListener("click", () => {
      if (state.isReview) {
        ...
        const types = getSelectedTypes();
        const qs = YuwenGen.generateQuestions(types.length ? types : null, getLevel(), getCount());
        startRound(qs, false, getLevel());
      }
    });
